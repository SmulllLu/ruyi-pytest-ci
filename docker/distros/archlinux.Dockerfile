...
RUN rm /etc/pacman.conf.d/*noextract*.conf && pacman-key --init && pacman --noconfirm -Syyu && pacman --need --noconfirm -S git python python-pexpect python-pytest glibc-locales sudo make jq tar unzip
