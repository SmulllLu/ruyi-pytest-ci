...
RUN dnf upgrade -y && dnf install -y git python3 python3-pexpect python3-pytest coreutils util-linux grep procps bash sudo wget make zstd xz unzip jq glibc-locale-source glibc-langpack-en glibc-langpack-zh
RUN echo 'LANG=en_US.UTF-8' > /etc/locale.conf
RUN mkdir -p /usr/lib/locale && localedef --no-archive -i en_US -f UTF-8 en_US.UTF-8 && localedef --no-archive -i zh_CN -f UTF-8 zh_CN.UTF-8
