...
RUN apt-get update && apt-get upgrade -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" && apt-get install -y git python3 python3-pexpect python3-pytest coreutils util-linux sudo make tar jq build-essential locales tzdata wget zstd lz4 unzip && apt-get clean
